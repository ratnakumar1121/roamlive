...
// --- Rate Limiter for Authentication Routes --- // <-- NEW SECTION
const authLimiter = rateLimit({
	windowMs: 15 * 60 * 1000, // 15 minutes
	max: 20, // Limit each IP to 20 requests per window (adjust as needed)
	message: { // Send JSON response on limit reached
        status: 429, // Too Many Requests
        message: 'Too many login/signup attempts from this IP, please try again after 15 minutes'
    },
	standardHeaders: true, // Return rate limit info in the `RateLimit-*` headers
	legacyHeaders: false, // Disable the `X-RateLimit-*` headers
    // store: // Optional: Use an external store like Redis for distributed environments
});

// Apply the limiter ONLY to login and signup routes
app.use('/api/auth/login', authLimiter);
app.use('/api/auth/signup', authLimiter);
// --- END of Rate Limiter Section --- //
